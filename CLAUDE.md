...
```javascript
{
  ...
  scores: [
    ...
    { label: '市场叙事/赛道溢价', pts: 0, max: 15, desc: '...' },  // ⚠️ 必填，满分115，不得省略
  ],
  financial: [
...
### ⚠️ 新股入库前强制检查清单（每只必过，不得跳过）

每次完成分析、写入 `index.html` 前，逐项核对：

**A · 评分完整性**
- [ ] scores 数组是否包含**5个维度**（业务质量/财务健康/估值吸引力/资本结构/市场叙事赛道溢价）
- [ ] score 字段值 = 五维合计，满分上限为 **115**，不是 100
- [ ] 市场叙事/赛道溢价是否按当前市场热度客观评分（热门AI/机器人/低空赛道不得低于12）

**B · 硬性触发规则核查**
- [ ] 是否 A+H 股？H股相对A股折价是否 >30%？→ 是则直接打
- [ ] 是否 18C 特专科技？基石中是否有顶级机构（贝莱德/高盛/摩根等）领投且基石占比 >30%？→ 是则最低1手彩票，不得结论为不打
- [ ] 认购倍数待定（上市前2-4日复核）→ >200x 升级为积极打

**C · 特殊类型识别**
- [ ] 确认是否属于：18C / 18A / A+H / WVR / 转板（isTransfer）
- [ ] 18C：第五维度打满12-15；1手彩票下限
- [ ] 18A：关注临床里程碑、EMA/FDA在审状态
- [ ] A+H：折价率是否已计算并填入 sector 字段

**D · 字段一致性**
- [ ] verdict / verdictLabel / score / position 四字段是否与五维总分对应（≥70=da, 55-69=wait轻仓, 40-54=wait观望, <40=no）
- [ ] applyEndTs 和 listTs 是否来自招股书（禁用 Futu API 时间戳）
- [ ] greenshoe 是否同时搜索了「超額配股權」和「超額配售」两个关键词
